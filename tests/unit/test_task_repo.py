...
async def test_list_tasks(session: AsyncSession) -> None:
    ...
    task1 = Task(name=fake.name(), description=None, priority=TaskPriority.LOW, status=TaskStatus.NEW)
    task2 = Task(name=fake.name(), description=None, priority=TaskPriority.MEDIUM, status=TaskStatus.NEW)
...
async def test_list_tasks_by_status(session: AsyncSession) -> None:
    ...
    task = Task(name=fake.name(), description=None, priority=TaskPriority.MEDIUM, status=TaskStatus.PENDING)
...
async def test_update_task(session: AsyncSession) -> None:
    ...
    task = Task(name=fake.name(), description=None, priority=TaskPriority.HIGH, status=TaskStatus.NEW)
    created = await repo.create(task)
